...
setCartCount(0);
  ...
  return (
    <div style={{ padding: "20px" }}>
      <h2 style={{marginBottom:"5%",marginLeft:"10%"}}>User Cart</h2>
...
<div>
      {cartItems.map((item) => (
        <div key={item.id} className="cartItem">
          ...
          <div className="quantityBtnsCon">
          <button className="quantityBtns" onClick={() => updateQuantity(item.id, item.quantity - 1)} disabled={item.quantity <= 1}>-</button>
          <button className="quantityBtns" onClick={() => updateQuantity(item.id, item.quantity + 1)}>+</button>
          </div>
          </div>
        </div>
      ))}
      ...
      {cartItems.length > 0 && (
        <>
        <div className="amountBtm">
          <h3>Total amount  ₹{total}</h3>
          <button onClick={handleCheckout} id="payBtn">Pay with Razorpay</button>
          </div>
        </>
      )}
    </div>
